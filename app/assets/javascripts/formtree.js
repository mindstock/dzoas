	...
	if (nodes.length > 0) {
		...
		if (startNode !=0) {
			...
			if (nodeValues[3].indexOf("#")>-1){
				var ss="<a href='#gotop' onclick=\"parent.seldept('"+nodeValues[6]+"','"+nodeValues[2]+"','"+nodeValues[1]+ "','"+nodeValues[7]+ "')\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\"><img src=\"/images/orgtree/folderopen.gif\" align=\"absbottom\" alt=\"\" />" + nodeValues[2] + "</a><br />";
				document.write(ss);
			}else{
...
///////////////////////
			...
			if (nodeValues[3].indexOf("#")>-1){
			  if (nodeValues[1]!="1"){
		      document.write("<a href='#gotop' onclick=\"parent.seldept('"+nodeValues[6]+"','"+nodeValues[2]+"','"+nodeValues[1]+ "','"+nodeValues[7]+ "')\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\">");
			  }
			  else{
			     document.write("<input type=radio class=chkbox name=sel  value='"+nodeValues[4]+","+nodeValues[6]+","+nodeValues[7]+"'> <a href='#gotop' onclick=\"parent.seldept('"+nodeValues[7]+"','"+nodeValues[6]+"','"+nodeValues[1]+ "','"+nodeValues[7]+ "')\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\">");
			  }
			}else{
				document.write("<a href=\"" + nodeValues[3] + "\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\">");
			}
...
///////////////////////
			...
			// End link
			if (nodeValues[5]=="1"){
			document.write("</a><img class='specialimg' border='0' src='../../../images/delete_msg.gif' alt='删除信息集' onclick=\" delitem('"+nodeValues[7]+"','"+nodeValues[4]+"')\" ><br />");
			}
			else if (nodeValues[5]=="2"){
			document.write(":<font color=red>已发布</font></a><br />");
			}
            else{
			document.write("</a><br />");
			}
